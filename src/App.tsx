...
export default function App() {
  const appBasePathRaw = ((import.meta.env.VITE_APP_BASE_PATH as string) || '/').trim();
  const appBasePath = appBasePathRaw === '/'
    ? '/'
    : `/${appBasePathRaw.replace(/^\/+|\/+$/g, '')}`;
  const hostname = window.location.hostname.toLowerCase();
  const isEducationalPortal = hostname === 'learn.straddly.com' || hostname.startsWith('learn.');
